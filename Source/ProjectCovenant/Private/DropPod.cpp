...
{
	checkf(PlayerInputComponent, TEXT("Drop Pod InputComponent is null"))
...
void ADropPod::SetOccupant(ASubjectZero * NewOccupant)
{
	...
	if(OldOccupant)
	{
		...
		OldOccupant->GetCharacterMovement()->Velocity = GetVelocity() + FVector(0.f, 0.f, 2400.f);
...
void ADropPod::InputYaw(float Value)
{
	if(Value != 0.f)
	{
		...
		AddControllerYawInput(GetWorld()->GetDeltaSeconds() * Value * Sensitivity);
	}
}
void ADropPod::InputPitch(float Value)
{
	if(Value != 0.f)
	{
		...
		AddControllerPitchInput(GetWorld()->GetDeltaSeconds() * Value * Sensitivity);
	}
}
